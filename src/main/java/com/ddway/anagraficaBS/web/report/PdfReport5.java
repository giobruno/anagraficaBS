...
public class PdfReport5 extends AbstractPdfView {
    ...
    protected void buildPdfDocument(Map<String, Object> stringObjectMap, Document document, PdfWriter pdfWriter, HttpServletRequest request, HttpServletResponse response) throws Exception {
		logger.info("Inizio metodo PdfReport5.buildPdfDocument!");
       
		List<ModelApplicativoPiuBSAssoicatiBean> modelApplicativiList =  (List<ModelApplicativoPiuBSAssoicatiBean>) request.getSession().getAttribute("modelApplicativiList");	   
		request.getSession().removeAttribute("modelApplicativiList");
		    ...
		    try{
		        ...
		        PdfPTable tabellaReport1 = caricaTabellePdf.caricaTabellaReport5(modelApplicativiList);
		        document.add(tabellaReport1);		        		       			  
		    }catch(Exception e){
		        e.printStackTrace();
		        logger.error(e.getMessage()+" on method PdfReport5.buildPdfDocument!");
		        throw e;
		    }
		    document.close();
		    }
