...
public class PdfReport1 extends AbstractPdfView {
	
	private static final Logger logger = LoggerFactory.getLogger(PdfReport1.class);
    ...
    protected void buildPdfDocument(Map<String, Object> stringObjectMap, Document document, PdfWriter pdfWriter, HttpServletRequest request, HttpServletResponse response) throws Exception {
		logger.info("Inizio metodo PdfReport1.buildPdfDocument!");
		...
		List<BusinessServiceBean> businessServicesList = (List<BusinessServiceBean>) request.getSession().getAttribute("businessServicesList");	   
		    ...
		    try{
		        ...
		        PdfPTable tabellaReport1 = caricaTabellePdf.caricaTabellaReport1(businessServicesList);
		        document.add(tabellaReport1);		        		       			  
		    }catch(Exception e){
		        e.printStackTrace();
		        logger.error(e.getMessage()+" on method PdfReport1.buildPdfDocument!");
		        throw e;
		    }
		    document.close();
		    }
